...
@implementation NSString (YJSString)

+ (NSString *)stringEmpty:(NSString *)emptypString{
    NSString *string = [NSString stringWithFormat:@"%@", emptypString];
    if(string == nil){
        return @"";
    }
    ...
    if([string isEqualToString:@""] || [string isEqualToString:@"(null)"] || [string isEqualToString:@"<null>"] || [string isEqualToString:@"null"]){
        return @"";
    }
...
@implementation NSString (YJSBase64)
...
+ (NSString *)HloveyRC4:(NSString*)aInput key:(NSString*)aKey {
    NSMutableArray *iS = [[NSMutableArray alloc] initWithCapacity:256];//char[] cypherBytes = new char[256];
    NSMutableArray *iK = [[NSMutableArray alloc] initWithCapacity:256];//char[] keyBytes = new char[256];
    ...
    for (short x=0; x<[aInput length]; x++) {
        ...
        int is = [[iS objectAtIndex:i] intValue];//char tmp = cypherBytes[i];
        j = (j+is)%256;//jump = (jump + tmp) & 0xFF;
        
        int is_i = [[iS objectAtIndex:i] intValue];
        int is_j = [[iS objectAtIndex:j] intValue];
        
        int t = (is_i+is_j) % 256;//char t = (char) ((tmp + cypherBytes[jump]) & 0xFF);
...
@implementation NSString (YJSSize)
